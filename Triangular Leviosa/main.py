...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        r1 = self.Map.create_image(295, 147, image=self.node_image)      #(350, 82)(281, 235)(400, 235)
        r2 = self.Map.create_image(295, 396, image=self.node_image)
        r3 = self.Map.create_image(418, 271, image=self.node_image)
        #r4 = self.Map.create_image(200, 329, image=self.node_image)
        #r5 = self.Map.create_image(200, 408, image=self.node_image)
        #r6 = self.Map.create_image(350, 375, image=self.node_image)

        self.relays = [(Relay(r1, 295, 147, "Relay 1", 0, distanceToUnits(150))), #meters 232 -> units 290.87
                       (Relay(r2, 295, 396, "Relay 2", 1, distanceToUnits(150))),
                       (Relay(r3, 418, 271, "Relay 3", 2, distanceToUnits(150)))]
                       #(Relay(r4, 200, 329, "Relay 4", 3, distanceToUnits(150))),
                       #(Relay(r5, 200, 408, "Relay 5", 4, distanceToUnits(150))),
                       #(Relay(r6, 350, 375, "Relay 6", 5, distanceToUnits(150)))]
        ...
        self.Map.create_text(self.relays[0].x_pos, self.relays[0].y_pos, text=f"{1}", font=("Roboto Medium", -20))
        self.Map.create_text(self.relays[1].x_pos, self.relays[1].y_pos, text=f"{2}", font=("Roboto Medium", -20))
        self.Map.create_text(self.relays[2].x_pos, self.relays[2].y_pos, text=f"{3}", font=("Roboto Medium", -20))
        #self.Map.create_text(self.relays[3].x_pos, self.relays[3].y_pos, text=f"{4}", font=("Roboto Medium", -20))
        #self.Map.create_text(self.relays[4].x_pos, self.relays[4].y_pos, text=f"{5}", font=("Roboto Medium", -20))
        #self.Map.create_text(self.relays[5].x_pos, self.relays[5].y_pos, text=f"{6}", font=("Roboto Medium", -20))
        self.next_relay = 0
    ...
    def locate2(self):
        ...
        if Rssi[1] > Rssi[5]:
            y1, x1 = (self.relays[1].x_pos - self.relays[1].x_pos), (self.relays[1].y_pos - self.relays[1].y_pos)
            y2, x2 = (self.relays[2].x_pos - self.relays[1].x_pos), (self.relays[2].y_pos - self.relays[1].y_pos)
            y3, x3 = (self.relays[0].x_pos - self.relays[1].x_pos), (self.relays[0].y_pos - self.relays[1].y_pos)
            r1 = self.relays[1].get_ToF()
            r2 = self.relays[2].get_ToF()
            r3 = self.relays[0].get_ToF()
        else:
            y1, x1 = self.relays[4].x_pos - self.relays[4].x_pos, self.relays[4].y_pos - self.relays[4].y_pos
            y2, x2 = self.relays[3].x_pos - self.relays[3].x_pos, self.relays[3].y_pos - self.relays[3].y_pos
            y3, x3 = self.relays[5].x_pos - self.relays[5].x_pos, self.relays[5].y_pos - self.relays[5].y_pos
            r1 = self.relays[4].get_ToF()
            r2 = self.relays[3].get_ToF()
            r3 = self.relays[5].get_ToF()
    ...
    def locate(self):
        # right 3(418, 271) top 1(295, 147) bottom 2(295, 396)
        # right(123, 124) top(0  , 0  ) bottom(0  , 249)
        y1, x1 = 0, 0
        y2, x2 = self.relays[1].x_pos - self.relays[0].x_pos, self.relays[1].y_pos - self.relays[0].y_pos
        y3, x3 = self.relays[2].x_pos - self.relays[0].x_pos, self.relays[2].y_pos - self.relays[0].y_pos
...
#def locate(self, anchor, second, third):

        #for relay in self.relays:
        #    if relay != anchor or relay != second or relay != third:
        #        self.Map.delete(self.ovals[relay.count])
        #    if anchor.x_pos == relay.x_pos and anchor.y_pos == relay.y_pos:
        #        anchor = relay
        #    if anchor.y_pos == relay.y_pos and anchor.x_pos != relay.x_pos:
        #        second = relay
        #    if anchor.x_pos != relay.x_pos and anchor.y_pos != relay.y_pos:
        #        third = relay
        ...
        # right(123, 124) top(0  , 0  ) bottom(0  , 249)
        #y1, x1 = 0, 0
        #y2, x2 = second.x_pos - anchor.x_pos, second.y_pos - anchor.y_pos
        #y3, x3 = third.x_pos - anchor.x_pos, third.y_pos - anchor.y_pos
        #r1 = anchor.get_ToF()
        #r2 = second.get_ToF()
        #r3 = third.get_ToF()
        #ic((x1, y1), r1, (x2, y2), r2, (x3, y3), r3)
    ...
    def serial_poll(self):
        ...
        for relay in self.relays:
            tof = round(np.abs((random.randint(-40, -30) * -6.75675 - 358.78378)))
            relay.set_ToF(tof)
            text = relay.name + f" ( {relay.x_pos}, {relay.y_pos} ) RSSI: {relay.get_ToF()}"
            if self.relayLabels[relay.count] != 0:
                self.relayLabels[relay.count].config(text=text)
            self.Map.delete(self.ovals[relay.count])
            self.ovals[relay.count] = self.Map.create_oval(relay.x_pos - relay.get_ToF(), relay.y_pos - relay.get_ToF(), relay.x_pos + relay.get_ToF(), relay.y_pos + relay.get_ToF(), width=1)
        self.locate()#self.relays[0])
        ...
        #         if inlen > 0:
        #             ic(inlen)
        #             data = base.read_until(expected=b'\n')[:-2]
        #             message = base.decode('utf-8')
        ...
        #                     RSSI_s = s.split(',')
        #                     RSSI = int(RSSI_s[1])
        #                     distance = round(np.abs(-6.75675 * RSSI - 358.78378))
        #                     ic(distance)
        #                     self.relays[int(RSSI_s[0])].set_ToF(distance)
        #                 else:
        #                     #self.Map.delete(self.ovals[0])
        #                     #self.Map.delete(self.ovals[1])
        #                     #self.Map.delete(self.ovals[2])
        #                     self.locate()#self.relays[0], self.relays[4], self.relays[5])
